...
public class Day4_class_instance {
	...
	public Day4_class_instance(int initialAge) {
  		if(initialAge<0)
          {
              age=0;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < T; i++) {
			int age = sc.nextInt();
			Day4_class_instance p = new Day4_class_instance(age);
			p.amIOld();
			for (int j = 0; j < 3; j++) {
				p.yearPasses();
			}
...
initialAge,the constructor should set age to 0 and print Age is not valid, 
...
Output Format
Complete the method definitions provided in the editor so they meet the specifications
outlined above; the code to test your work is already in the editor.
If your methods are implemented correctly, each test case will print 2 or 3 lines
(depending on whether or not a valid intialAge was passed to the constructor).
...
message followed by the young message. 
...
Three years pass and age=19, so we print that the person is now a teenager.
